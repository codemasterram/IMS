...
var
	...
	detach = function() {
		if ( document.addEventListener ) {
			document.removeEventListener( "DOM~/ContentLoaded", completed, false );
...
jQuery.ready.promise = function( obj ) {
	if ( !readyList ) {
		...
		// Standards-based browsers support DOM~/ContentLoaded
			...
			document.addEventListener( "DOM~/ContentLoaded", completed, false );
					...
					for ( i = 0; i < hasScripts; i++ ) {
						...
						if ( rscriptType.test( node.type || "" ) &&
							...
							if ( node.src ) {
								...
								jQuery.globalEval( ( node.text || node.text~/Content || node.innerHTML || "" ).replace( rcleanScript, "" ) );
							}
						}
					}
				}
		...
		if ( !s.crossDomain || jQuery.support.cors ) {
			...
			return {
				send: function( headers, complete ) {
					...
					xhr.send( ( s.has~/Content && s.data ) || null );
