...
namespace SoftIms.Utilities
{
    public static class ViewHelper
    {
        ...
        {
            Type type = typeof(TModel); MemberExpression memberExpression = (MemberExpression)expression.Body; string propertyName = ((memberExpression.Member is PropertyInfo) ? memberExpression.Member.Name : null); DisplayAttribute attr = (DisplayAttribute)type.GetProperty(propertyName).GetCustomAttributes(typeof(DisplayAttribute), true).SingleOrDefault(); if (attr == null) { MetadataTypeAttribute metadataType = (MetadataTypeAttribute)type.GetCustomAttributes(typeof(MetadataTypeAttribute), true).FirstOrDefault(); if (metadataType != null) { var property = metadataType.MetadataClassType.GetProperty(propertyName); if (property != null) { attr = (DisplayAttribute)property.GetCustomAttributes(typeof(DisplayAttribute), true).SingleOrDefault(); } } } var displayName = attr != null ? attr.Name : propertyName; return $"{pushText} {displayName} {pushText}".TrimStart().TrimEnd(); }
        ...
        public static SelectList GetEmployeeList(int? departmentId, object selectedValue = null)
        {
            using (UnitOfWork db = new UnitOfWork())
            {
                var data = db.EmployeeRepo.Table().Where(x => x.DepartmentId == departmentId).OrderBy(x => x.Name);
